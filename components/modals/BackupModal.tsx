      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div className="space-y-4">
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-6">
                <p className="text-xs text-slate-500 italic">{t(nativeLang, 'cloud_help')}</p>
                ...
                <div>
                    <button
                        ...
                        disabled={isLoading}
                        className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-md shadow-indigo-200 disabled:opacity-70 mb-3"
                    >
                    ...
                    {generatedUrl && (
                        <div className="bg-white p-3 rounded-xl border border-indigo-100 animate-in fade-in">
                            <p className="text-xs font-bold text-green-600 mb-1 flex items-center gap-1"><Check size={12} /> {t(nativeLang, 'upload_success')}</p>
                            <div className="flex gap-2">
                                <input 
                                    type="text" 
                                    readOnly 
                                    value={generatedUrl} 
                                    className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-slate-600 outline-none select-all"
                                />
                                <button 
                ...
                <div className="border-t border-slate-200 pt-4">
                     <label className="text-xs font-bold text-slate-700 mb-2 block">Khôi phục dữ liệu:</label>
                     <div className="flex gap-2">
                        <div className="relative flex-1">
                            ...
                            <input 
                                ...
                                className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                            />
                        </div>
                        <button 
                            onClick={handleDownload}
                            disabled={isLoading || !inputUrl.trim()}
                            className="px-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl transition-all disabled:opacity-50"
                        >
