...
export const ActiveLesson: React.FC<ActiveLessonProps> = ({
  ...
  return (
    <div className="flex flex-col h-full relative bg-slate-50">
      {/* Header */}
      <header className="bg-white border-b border-slate-100 p-4 flex items-center justify-between sticky top-0 z-10 shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="font-bold text-slate-900 text-sm md:text-base max-w-[150px] truncate">{config.topic}</h1>
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        {isReadingMode ? (
          ...
          <div className="flex flex-col h-full">
            ...
            <div className={`bg-slate-100 border-b border-slate-200 transition-all duration-300 ease-in-out overflow-hidden ${showReadingContext ? 'max-h-40' : 'max-h-0'}`}>
            ...
            <button 
               ...
               className="w-full flex items-center justify-center py-1 bg-white border-b border-slate-100 text-[10px] font-bold text-slate-400 hover:bg-slate-50 uppercase tracking-wider"
            >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 bg-gradient-to-b from-indigo-50 to-white text-center relative overflow-y-auto">
               <button 
                   onClick={() => setShowTranslation(!showTranslation)}
                   className="absolute top-4 right-4 p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-all z-10"
                   title={showTranslation ? "Ẩn bản dịch" : "Hiện bản dịch"}
               >
                   {showTranslation ? <Eye size={20} /> : <EyeOff size={20} />}
               </button>

               <div className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-3 shrink-0">Dịch câu sau sang tiếng {config.targetLang === 'ko' ? 'Hàn' : config.targetLang === 'en' ? 'Anh' : 'Nhật'}</div>
               {currentPuzzle ? (
                 <div className={`transition-all duration-300 w-full max-w-2xl ${showTranslation ? 'opacity-100 blur-0' : 'opacity-30 blur-md select-none'}`}>
                    <h2 className="text-lg md:text-2xl font-bold text-slate-800 leading-relaxed animate-in zoom-in duration-300 px-2">
                        "{currentPuzzle.targetAnswerTranslation}"
                    </h2>
                 </div>
               ) : (
                 <div className="animate-pulse h-8 w-48 bg-slate-200 rounded-lg"></div>
               )}
            </div>
      ...
      <div className={`${isReadingMode ? 'h-1/2 border-t-4 border-indigo-100' : 'h-auto border-t border-slate-100'} bg-white p-4 shrink-0 z-20 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] flex flex-col justify-center`}>
        {currentPuzzle ? (
          <WordPuzzle
            ...
            // In Reading mode, the prompt is the Big Text in the top half, so we don't show the small prompt box inside the puzzle.
